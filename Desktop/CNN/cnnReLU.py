...
def loadData():
  ...
  def loadFile(fileName, mode="rb"):
    with open(fileName, mode) as raw:
      ...
      return parsed
  data = {"train":[], "test":[]}
  trainImages = loadFile("/Users/michaelpilarski/Desktop/CNN/Data/train-images-idx3-ubyte")
  trainLabels = loadFile("/Users/michaelpilarski/Desktop/CNN/Data/train-labels-idx1-ubyte")
  data["train"] = np.asarray(list(zip(trainImages, np.asarray([vectorize(i) for i in trainLabels]))))
  testLabels = loadFile("/Users/michaelpilarski/Desktop/CNN/Data/t10k-labels-idx1-ubyte")
  testImages = loadFile("/Users/michaelpilarski/Desktop/CNN/Data/t10k-images-idx3-ubyte")
...
class Network():
  ...
  def updateMiniBatch(self, miniBatch, lmbda, eta, momentum):
    weightError = np.copy(self.weights) 
    biasError = np.copy(self.biases)
    betaWeightError = np.copy(weightError)
    betaBiasError = np.copy(biasError)
    for image in miniBatch:
      ...
      #betaWeightError = [momentum*bwe+(1-momentum)*we for bwe, we in zip(betaWeightError, weightError)]
      #betaBiasError = [momentum*bbe+(1-momentum)*be for bbe, be in zip(betaBiasError, biasError)]
    #self.weights = [(1-eta*lmbda/len(trainingData))*w-(((float(eta)/len(miniBatch))*we)) for w, we in zip(self.weights, betaWeightError)]
    #self.biases = [b-(((float(eta)/len(miniBatch))*be)) for b, be in zip(self.biases, betaBiasError)]
  ...
  def SGD(self, lmbda, eta, trainingData, testData, epochs, miniBatchSize, momentum):
    for j in range(epochs):
      start = times()
      random.shuffle(trainingData)
      miniBatches = [trainingData[k:k+miniBatchSize] for k in range(0, len(trainingData), miniBatchSize)]
      for miniBatch in miniBatches: self.updateMiniBatch(miniBatch, lmbda, eta, momentum)
      totalCorrect = 0
      totalPercent = 0
      evaluateStart = times()
      random.shuffle(trainingData)
...
network.SGD(.01, .01, trainingData, testData, 20, 10, .9)
#network.display()
